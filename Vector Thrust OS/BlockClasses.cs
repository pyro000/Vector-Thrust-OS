    ...
    partial class Program
    {
		...
		class VectorThrust
		{
			...
			public void Go()
			{
				...
				if (program.parked || !TO)
				{
					double anglecosfixed = angleCosPercent <= 0 ? 1 : angleCosPercent;
					finalrpm = (float)(0.1 * 17000 / anglecosfixed);
					if (!program.parked) finalrpm = MathHelper.Clamp(finalrpm, 1, 30);
				}
			...
			/*public float RotateThrusterTowards(Vector3 target, IMyThrust th, IMyMotorStator rt) //rotates a thruster as close as it can go towards a vector
			{
				//target is WORLD ALIGNED NOT LOCAL!!!! Bruh
				...
				float TargetRot = Vector3.Dot(TorqueVector, rt.WorldMatrix.Up);
				...
				float Correction = (float)c.ControlVariable(program.Runtime.TimeSinceLastRun);

				program.Echo(""+rt.CustomName);
				program.Echo("Cor:" + Correction.NNaN());

				program.Write("Cor:" + Correction.NNaN());

				rt.TargetVelocityRad = Correction.NNaN();
